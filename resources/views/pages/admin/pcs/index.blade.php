                ...
                <div class="col-12 col-md-12 col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>List pcs</h4>
                            <a href="{{ route('admin.pcs.create') }}" class="btn btn-primary">Tambah pcs</a>
                        </div>
                        <div class="card-body">
                            @if(session('success'))
                                ...
                                <table class="table table-bordered table-md mb-4">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $pc->ram }}</td>
                                            <td>{{ $pc->storage }}</td>
                                            <td>@if($pc->status == 'available')
                                                <span class="badge badge-success">Available</span>

                                                @elseif($pc->status == 'borrowed')
                                                <span class="badge badge-warning">Dipinjam</span>

                                                @elseif($pc->status == 'maintenance')
                                                <span class="badge badge-danger">Maintenance</span>
                                                @else
                                                <span class="badge badge-secondary">{{ ucfirst($pc->status) }}</span>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.pcs.edit', $pc->id) }}" class="btn btn-sm btn-warning">Edit</a>
                                                <form action="{{ route('admin.pcs.destroy', $pc->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
